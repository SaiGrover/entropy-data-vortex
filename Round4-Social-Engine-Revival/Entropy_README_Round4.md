...
Run from **inside this folder** - the app walks up to the repository root to read the other rounds'
artefacts, and `static/` and `.streamlit/` resolve relative to the script.

```bash
cd Round4-Social-Engine-Revival
pip install -r Entropy_requirements.txt
streamlit run Entropy_app.py
```

Opens at `http://localhost:8501`. For SHANNON, copy `.env.example` to `.env` and set `GROQ_API_KEY`.
The theme is pinned in `.streamlit/config.toml` so the dashboard renders identically regardless of the
viewer's OS theme. It is laid out for a desktop window; below roughly 900 px wide Streamlit stacks the
columns and the chart grid becomes a single column.

## Deploy to Streamlit Community Cloud

| Field | Value |
|---|---|
| Repository | `SaiGrover/entropy-data-vortex` |
| Branch | `main` |
| Main file path | `Round4-Social-Engine-Revival/Entropy_app.py` |
| Advanced -> Python version | **3.12** |
| Advanced -> Secrets | `GROQ_API_KEY = "gsk_..."` (see `.streamlit/secrets.toml.example`) |

Cloud installs the **root** `requirements.txt` and reads the **root** `.streamlit/config.toml`, so both live
at the top of the repository as well as here. Python 3.12 matters: the model libraries are marked to install
only on 3.12 Linux, where torch comes from the CPU-only wheel. On any other version they are skipped and the
two live scoring demos show a short note instead - the rest of the dashboard is unaffected.

The first build is slow: the repository is about 270 MB, most of it the Round 2 fine-tuned seeds and the
bundled sentence encoder.
...
|---|---|---|
| **Round 1 P1 - Data Recovery** | The repair ledger, platform volume, engagement distributions, the correlation check, spread by coefficient of variation, the midnight-timestamp artefact, followers vs engagement, and three hypothesis tests, filtered live by **platform, date range and follower band** so a reader can try to break the flatness claim themselves | Recomputes everything from the CSVs, filtered live by the **platform multi-select** at the top of the page |
| **Round 1 P2 - Analytical Core** | The constrained schema and the three submitted questions (E3, M4, H4) in **nested tabs**, each with its result chart, result table and the SQL itself, plus a **read-only SQL console** with four starter questions, the result table and the query plan | **Executes the three `.sql` files against `Entropy_social_engine.db` in read-only mode** on every page load |
| **Round 2 - Semantic Recovery** | Every candidate ranked by validation and test macro-F1, the confusion matrix, per-class precision/recall/F1, the substring-rule discovery, and a live "read a post" demo, and an **error explorer** over all 1,185 held-out test posts - filter by labelled and predicted class, confidence and text to read the model's actual mistakes | Loads the saved ensemble (TF-IDF + embeddings + 3 MiniLM seeds) and scores text you type |
...
- **Every page lets you test its claim, not just read it.** Round 1 Phase 1 can be sliced by platform,
  date range and follower band, with a live readout of platform-share spread and engagement CV, so the
  flatness finding can be challenged directly. Round 1 Phase 2 has a read-only SQL console against the real
  database: `SELECT`/`WITH` only, one statement, 500-row cap, a read-only URI, and a progress-handler abort so
  a runaway query cannot hang the page, with the query plan shown beside every result. Round 2 has an error
  explorer over the 1,185 held-out test posts, which opens on the 330 the model gets wrong (27.8%) - where it
  is immediately visible that most errors are mild sentiment landing on the wrong side of a class boundary.
- **The live model runs when deployed, not just locally.** Three things stood between the saved ensemble and a hosted server, and each is handled. The Round 2 config names its sentence encoder by Hugging Face Hub id, so a fresh server would download it and depend on the Hub being up; a copy is bundled in `models/all-MiniLM-L6-v2/` and the loader goes offline when it is present, producing bit-identical predictions. The pickles were written by scikit-learn 1.8.0, so Cloud installs exactly that. And torch comes from the CPU-only wheel, because a plain `torch` on Linux pulls several GB of CUDA libraries.
- **It cannot take the app down.** Loading the stack costs about 750 MB (measured: ~370 MB to import torch, ~360 MB for the ensemble). A container that runs out of memory is killed outright, which no `except` can catch, so the loader checks the cgroup memory limit first and declines to load rather than risk it. Any other failure is caught and cached for the whole server, so every later visitor sees a short note at once instead of waiting a minute to rediscover it. Presence of the libraries is checked with `find_spec`, not an import, so rendering a page never pays the 370 MB.
- **SHANNON, an assistant that only knows this project.** *Retrieval-Augmented Intelligence by Entropy* - named for the author of information theory, which is also where the team name comes from. A dock at the bottom left answers questions from the repository itself - the four READMEs, every report's LaTeX source, the metrics and robustness JSON and the cleaning log - retrieved with a hybrid word/character TF-IDF index and summarised by a Groq-hosted model, with the source files listed under every answer. It needs `GROQ_API_KEY` - in a `.env` beside `Entropy_app.py` locally, or in `st.secrets` when deployed (both are read); without one it says so rather than failing. The model is resolved against what the key can actually reach rather than hard-coded, since Groq's catalogue varies by account.
...
|-- models/all-MiniLM-L6-v2/       # bundled sentence encoder, so the model needs no network
|-- Entropy_requirements.txt       # pinned minimum versions
|-- .streamlit/config.toml         # pinned deep-forest theme + static serving
|-- .streamlit/secrets.toml.example  # what to paste into Cloud's Secrets box
...
- The live scoring demos load three fine-tuned transformers and a sentence encoder (about 750 MB, roughly ten seconds warm, longer on a cold server); the result is cached for the life of the server. Every other page renders in under a second.
