...
def fetch_post(url: str) -> dict:
    ...
    try:
        ...
        if "reddit.com" in host and e.code in (403, 429):
            raise FetchError("Reddit refused the request. It blocks many requests coming from cloud "
                             "servers, so Reddit links often fail on the hosted dashboard even though they "
                             "work locally. Try a Hacker News, Mastodon or Lemmy link, or paste the post's "
                             "text into the box directly.")
