...
def model_available() -> bool:
    ...
    Checked with find_spec rather than an import: importing torch costs about
    370 MB, and this runs on every render of two pages whether or not anyone
...
# Measured on this ensemble: importing the torch stack adds ~370 MB and loading
# the three fine-tuned seeds plus the sentence encoder adds ~360 MB more.
MODEL_NEEDS_MB = 900


def free_memory_mb():
    """Headroom before this process would be killed, or None if unreadable.

    A container that runs out of memory is killed outright - no exception, no
    except clause, just a restarted app. So the only graceful failure is to
    look first. Reads the cgroup limit (what a hosted container actually
    enforces), then /proc/meminfo, then psutil.
    """
    try:
        for lim_f, use_f in (("/sys/fs/cgroup/memory.max", "/sys/fs/cgroup/memory.current"),
                             ("/sys/fs/cgroup/memory/memory.limit_in_bytes",
                              "/sys/fs/cgroup/memory/memory.usage_in_bytes")):
            if os.path.exists(lim_f) and os.path.exists(use_f):
                lim = open(lim_f).read().strip()
                if lim != "max" and int(lim) < (1 << 50):
                    return (int(lim) - int(open(use_f).read().strip())) / 1048576
...
@st.cache_resource(show_spinner=False)
def _model_state():
    """One shared record per server process, so a failure is remembered for
    every visitor rather than each of them waiting to rediscover it."""
    return {"error": ""}
...
def model_broken() -> str:
    """Why loading failed, if it has - on this server, not just this session."""
    return _model_state()["error"] or st.session_state.get("model_error", "")
...
<style>
  ...
  .linklist .caveat {{ color:{AMBER}; font-size:.68rem; line-height:1.45; opacity:.92;
     margin-top:.25rem; padding-left:.5rem; border-left:2px solid rgba(217,178,106,.45); }}
  .emptyres {{ border:1px dashed rgba(190,210,192,.22); border-radius:14px; padding:1rem 1.1rem;
     margin-top:.6rem; background:rgba(255,255,255,.015); }}
  .er-title {{ font-family:'Outfit','Inter',sans-serif; font-weight:600; color:{MUT}; font-size:.95rem; }}
  .er-body {{ color:{DIM}; font-size:.8rem; line-height:1.6; margin-top:.3rem; max-width:560px; }}
  .er-bars {{ display:flex; align-items:flex-end; gap:12%; height:92px; margin:1rem 8% 0 8%;
     border-bottom:1px solid rgba(190,210,192,.14); }}
  .er-bars span {{ flex:1; border-radius:5px 5px 0 0; background:rgba(190,210,192,.07); }}
  .er-axis {{ display:flex; justify-content:space-between; margin:.35rem 8% 0 8%;
     color:{DIM}; font-size:.68rem; }}
  .er-axis span {{ flex:1; text-align:center; }}
...
@st.cache_resource(show_spinner=False)
def load_model_safe():
    """Load the ensemble once per server process, or explain why not.

    Returns (engine, utils) on success and (None, None) otherwise, with the
    reason stored in _model_state(). Both outcomes are cached, so a server that
    cannot hold the model says so instantly to every later visitor.
    """
    state = _model_state()
    free = free_memory_mb()
    if free is not None and free < MODEL_NEEDS_MB:
        state["error"] = (f"this server has about {free:.0f} MB free and the model needs roughly "
                          f"{MODEL_NEEDS_MB} MB, so it was not loaded rather than risk the whole app")
        return None, None
    try:
        return _load_model_impl()
    except MemoryError:
        state["error"] = "the server ran out of memory while loading it"
    except Exception as exc:
        state["error"] = f"loading it failed ({type(exc).__name__})"
    return None, None


BUNDLED_ENCODER = APP / "models" / "all-MiniLM-L6-v2"


def _load_model_impl():
    # The fine-tuned seeds live in the repo, but the Round 2 config names its
    # sentence encoder by Hugging Face Hub id, so a fresh server would download
    # it on first use and depend on the Hub being reachable. A copy is bundled
    # beside this file; when it is present the network is taken out of the path
    # entirely. The Round 2 artefacts themselves are left exactly as submitted.
    if BUNDLED_ENCODER.exists():
    ...
    if BUNDLED_ENCODER.exists():
        for comp in eng.cfg["components"]:
            if comp.get("kind") == "embedding":
                comp["embedder"] = str(BUNDLED_ENCODER)
    for comp in eng.cfg["components"]:
        obj = eng._load(comp)
...
if PAGE == 0:
    ...
    # 1,784 posts have no platform (Round 1 left them missing rather than
    # impute one), and isin() drops them - so only filter on platform once the
    # selection is actually narrowed, or the default view silently loses 15%
    narrowed = bool(pick) and set(pick) != set(plats)
    view = merged[merged.platform.isin(pick)] if narrowed else merged
...
if PAGE == 4:
    ...
    with pick_col:
        ...
        if st.button("Clear", key="r4_exclr", width="stretch"):
            ...
            st.session_state.pop("r4_src", None)
            st.session_state.pop("r4_result", None)
            st.rerun()
        ...
        st.markdown(
            ...
            '<div><b>Reddit</b><code>reddit.com/r/apple/comments/1wadiip/...</code>'
            '<span class="caveat">May fail on the hosted version - Reddit blocks many requests '
            'from cloud servers. Works when run locally.</span></div>'
    ...
    with work_col:
        ...
        # score on click, but keep the result in session state: a Streamlit button
        # is only True for one run, so without this the answer vanished the moment
        ...
        if run and st.session_state.r4_text.strip():
            try:
                with st.spinner("Loading the saved ensemble..."):
                    eng, nu = load_model_safe()
                if eng is None:
                    st.rerun()                      # the note now explains why
                text_in = st.session_state.r4_text.strip()
                proba, n_chunks = score_text(eng, text_in)
                topic, kwd, token, _ = nu.TopicRuleClassifier().explain(text_in)
                st.session_state.r4_result = {
                    "labels": list(eng.labels), "proba": [float(v) for v in proba],
                    "label": eng.labels[int(proba.argmax())], "words": len(text_in.split()),
                    "chunks": int(n_chunks), "topic": topic, "kwd": kwd, "token": token,
                    "text": text_in,
                }
            except Exception as exc:
                st.session_state.model_error = f"loading it failed ({type(exc).__name__})"
        ...
        res = st.session_state.get("r4_result")
        if res:
            lab, proba = res["label"], res["proba"]
            note = (f"{res['words']} words, scored in {res['chunks']} overlapping windows and averaged"
                    if res["chunks"] > 1 else f"{res['words']} words, short enough to score in one pass")
            via = (f" via &lsquo;{res['kwd']}&rsquo; inside &lsquo;{res['token']}&rsquo;"
                   if res["kwd"] else "")
            stale = res["text"] != st.session_state.r4_text.strip()
            res1, res2 = st.columns([1.1, 1], gap="medium")
            with res1:
                st.markdown(
                    f'<div class="panel" style="border-color:{SENT[lab]}66;height:100%">'
                    f'<div class="sub" style="margin:0">Predicted sentiment'
                    + (' <span style="color:#d9b26a">&middot; for the previous text - press Read it '
                       'again</span>' if stale else '') +
                    f'</div>'
                    ...
                    f'<div class="sub" style="margin:.15rem 0 0 0">{max(proba):.0%} confidence &middot; '
                    f'{note}</div>'
                    f'<div class="sub" style="margin:.35rem 0 0 0">Dataset topic rule reads this as '
                    f'<b>{res["topic"]}</b>{via} - a reminder that the Round 2 topic labels were a '
                    f'substring lookup, not semantics.</div></div>', unsafe_allow_html=True)
            with res2:
                fig = go.Figure(go.Bar(
                    x=res["labels"], y=proba, marker_color=[SENT[l] for l in res["labels"]],
                ...
                chart(fig_style(fig, 250, "What the engine actually returns", legend=False),
            ...
            # an empty state, so the column is not a dead block before the first run
            st.markdown(
                '<div class="emptyres">'
                '<div class="er-title">The result appears here</div>'
                '<div class="er-body">Pick an example on the left, paste a link, or write your own - '
                'then press <b>Read it</b>. You get the predicted sentiment, how confident the engine '
                'is, and all three class probabilities behind the call.</div>'
                '<div class="er-bars"><span style="height:38%"></span><span style="height:62%"></span>'
                '<span style="height:24%"></span></div>'
                '<div class="er-axis"><span>Negative</span><span>Neutral</span><span>Positive</span></div>'
                '</div>', unsafe_allow_html=True)
